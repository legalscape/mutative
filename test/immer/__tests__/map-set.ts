...
function runBaseTest(name, autoFreeze, useListener) {
  ...
  describe('map issues ' + name, () => {
    ...
    test('#466 - mapChangeBug ', () => {
      ...
      expect(applyPatches(obj, result[1])).toEqual(result[0]);
      expect(applyPatches(result[0], result[2])).toEqual(obj);
    });
    ...
    test('#466 - mapChangeBug2 ', () => {
      ...
      // !!! This is different from immer
      expect(applyPatches(obj1, result[1])).toEqual(result[0]);
      expect(applyPatches(result[0], result[2])).toEqual(obj1);
    });
